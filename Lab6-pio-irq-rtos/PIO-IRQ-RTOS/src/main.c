...
void but_callback(void) {
	int msg = -100;
	xQueueSendFromISR(xQueueButFreq, &msg, pdFALSE);
}
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
    if (xQueueReceive(xQueueButFreq, &msg, (TickType_t)0)) {
      delayTicks += msg;
      if (delayTicks < 100) delayTicks = 1000;
      if (delayTicks > 2000) delayTicks = 1000;
      printf("delay = %d \n", delayTicks);
...
int main(void) {
  ...
  if (xTaskCreate(task_but, "BUT1", TASK_BUT_STACK_SIZE, NULL,
                  TASK_BUT_STACK_PRIORITY, NULL) != pdPASS) {
    printf("Failed to create UartTx task\r\n");
  }
